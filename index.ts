...
const opts = {
  ...
  const fetchBikeTags = async (client: BikeTagClient) => {
    ...
    let {data: game} = await client.game(opts as unknown as getGamePayload)
    ...
    if (game || (opts.reddit?.subreddit || opts.imgur?.hash)) {
      ...
      const {data: sanityTag1} = await client.tags(1, {source: 'sanity'})
      const {data: redditTag1} = await client.tags(1, {source: 'reddit'})
    ...
    const {data: tags, source: tagsSource} = await client.tags()
